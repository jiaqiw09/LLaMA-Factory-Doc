...
LLaMA-Factory 支持FA，NpuFusedRMSNorm，NpuFusedSwiGlu，NpuFusedRoPE或NpuFusedMoE融合算子。

可在训练脚本中配置如下参数，模型加载后替换对应模型结构，使能NpuFusedRMSNorm，NpuFusedSwiGlu，NpuFusedRoPE和NpuFusedMoE融合算子，提升训练效率。
...
同时LLaMA-Factory 支持昇腾 NPU 的 Flash Attention (FA) 融合算子。在训练配置文件中设置：
...
.. list-table::
   ...
   * - 融合算子
     - 支持模型系列
   * - FA
     - Qwen3, Qwen3-MOE, Qwen3-VL, Qwen3-VL-MOE, Qwen3Omni, Qwen3Next, Deepseek-v2, Deepseek-v3
   * - NpuFusedRMSNorm
     - Qwen3, Qwen3-MOE, Qwen3-VL, Qwen3-VL-MOE, Qwen3Omni, Qwen3Next, Deepseek-v2, Deepseek-v3
   * - NpuFusedSwiGlu
     - Qwen3, Qwen3-MOE, Qwen3-VL, Qwen3-VL-MOE, Qwen3Omni, Qwen3Next, Deepseek-v2, Deepseek-v3
