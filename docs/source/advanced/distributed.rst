...
`FSDP2 <https://pytorch.org/tutorials/intermediate/FSDP_tutorial.html>`__ 在集成FSDP1基础功能的前提下，摒弃了 FSDP1 将参数压扁拼接的做法，转而基于 DTensor 实现逐参数切分，这一架构升级在完整保留模型原始结构的同时，显著提升了计算与通信的重叠效率，进而增强训练性能。如果想使用FSDP2，请保证 `torch >= 2.7`。
